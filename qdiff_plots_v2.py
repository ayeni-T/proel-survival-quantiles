...
SIM_METHODS         = ['proel', 'plel', 'na', 'bartlett']   # v4: added bartlett
...
def load_sim(path):
    ...
    for b in blocks:
        ...
        data.setdefault(n, {})[c] = {
            ...
            'width'   : b.get('width',    {}),
            'bartlett_bhat': b.get('bartlett_bhat', {}),   # v4 addition
        }
    return data
...
def _extract(data, metric, method, alpha, p, n, c):
    ...
    col = f'{method}_mean'
    try:
        val = data[n][c][metric][key][col]
    except KeyError:
        return np.nan
    return np.nan if val is None else val   # v4: bartlett fields can be
                                             # JSON null when out of scope
...
def _make_demo_sim():
    ...
    for n in SAMPLE_SIZES:
        ...
        for ic, c in enumerate(CENS_RATES):
            data[n][c] = {'coverage': {}, 'width': {}, 'bartlett_bhat': {}}
            for alpha in NOMINAL_ALPHAS:
                ...
                for p in QUANT_LEVELS:
                    ...
                    data[n][c]['coverage'][key] = {
                        ...
                        'bartlett_mean': float(np.clip(
                            nom + 0.003 + rng.normal(0, 0.004), 0.4, 1.0)),
                    ...
                    data[n][c]['width'][key] = {
                        ...
                        'bartlett_mean': float(base * (1.05 if (p==0.75 and c==0.4) else 1.02)),
                    }
                    if p == 0.75:
                        # b_hat ~ 1 at light/moderate censoring; inflated at
                        # heavy censoring for small n, decaying toward 1
                        bhat = 1.0 + (8.0 * (1 if c == 0.4 else 0.05)) * (30.0/n)**0.9
                        data[n][c]['bartlett_bhat'][key] = {
                            'mean': float(bhat), 'median': float(bhat*0.85), 'n': 2000}
    return data
...
def plot_figure3(rd_results):
    """
    Figure 3: Kaplan-Meier curves (raw, step function, group-colored)
    with the model-based (covariate-adjusted, Z=0 reference level)
    Breslow baseline survival curve overlaid as a black dotted line
    (same black color for both groups -- a deliberately distinct third
    visual category, not tied to group color) when available (requires
    the 'breslow' field added to analyse_dataset's output). Falls back
    gracefully to KM-only for results generated before that field
    existed.

    Legend per panel has three entries: the two group-colored KM
    lines, plus one black 'Model-based baseline' entry (not two, since
    the baseline color does not vary by group) -- fewer entries than an
    earlier attempt that tried to label both curve types per group
    (four entries), which was too crowded to read at a legible font
    size.
    """
    apply_style()
    ...
    for idx, (res, ax) in enumerate(zip(rd_results, axes)):
        ...
        for gk, color, lk in [('1',COLORS['group1'],'g1'),
            ...
            if breslow is not None and gk in breslow:
                bt = np.array(breslow[gk]['times'])
                bs = np.array(breslow[gk]['surv'])
                if len(bt) > 0:
                    ax.plot(bt, bs, color='black', lw=1.2,
                            ls=':', alpha=1.0, zorder=4)
                    t_max = max(t_max, bt[-1])
        ...
        if breslow is not None:
            from matplotlib.lines import Line2D
            handles = handles + [Line2D([0],[0], color='black', lw=1.2, ls=':')]
            labels  = labels  + ['Model-based baseline']
        ax.legend(handles, labels, loc=leg_loc, fontsize=7.2, framealpha=0.92,
...
# =====================================================================
# FIGURE 6 (v4 addition) — Bartlett correction factor (b_hat) vs n
# =====================================================================
# Directly visualizes the diagnostic finding underlying the Bartlett
# correction: b_hat (the mean of the bootstrap pivot, used to inflate
# the chi^2 cutoff) as a function of sample size, one line per
# censoring rate, at a chosen quantile level (default p=0.75, the
# quantile where the correction actually matters -- see
# FINDINGS_SUMMARY.md). Expect b_hat ~ 1 (little/no correction) at
# light/moderate censoring across all n, and a pronounced departure
# from 1 at heavy censoring for small/moderate n that shrinks back
# toward 1 as n grows -- reproducing, from the actual production run,
# the pattern established in the offline diagnostic grid.

def plot_figure6_bartlett_bhat(sim_data, p=0.75, alpha=0.05):
    apply_style()
    ...
    cens_markers = ['o', 's', '^']
    cens_styles  = ['-', '--', (0, (5,1,1,1))]
    for ic, (c, clabel) in enumerate(zip(CENS_RATES, CENS_LABELS)):
        y = []
        for n in SAMPLE_SIZES:
            try:
                y.append(sim_data[n][c]['bartlett_bhat'][key]['mean'])
            except KeyError:
                y.append(np.nan)
        y = np.array(y, dtype=float)
        ax.plot(x, y, color=COLORS['bartlett'], lw=LINEWIDTH['data'],
                ls=cens_styles[ic], marker=cens_markers[ic],
                ms=MARKERSIZE, label=f'Censoring {clabel}', zorder=3)

    ax.axhline(1.0, color=COLORS['nominal'], lw=LINEWIDTH['reference'],
               ls=LINESTYLE['nominal'], zorder=1)
    ax.set_xticks(SAMPLE_SIZES)
    ax.set_xlabel(XLABEL_N, fontsize=9)
    ax.set_ylabel(r'Bartlett factor $\hat{b}$', fontsize=9)
    ax.set_title(f'$p={p:.2f}$', fontsize=9, fontweight='bold', pad=6)
    ax.legend(loc='best', fontsize=7.5, framealpha=0.92,
              edgecolor='0.7', fancybox=False, borderpad=0.5,
              labelspacing=0.3)
    fig.tight_layout()
    return save_fig(fig, f'figure6_bartlett_bhat_p{int(p*100)}', tight=False)
...
if __name__ == '__main__':
    ...
    if not args.fig5_only:
        ...
        print('Figure 6 — Bartlett correction factor (b_hat) vs n...')
        print(f'  Saved: {plot_figure6_bartlett_bhat(sim_data, p=0.75)}')
